...
async def find_deleted_account_chats(client):
    """Return private dialogs whose user is marked deleted by Telegram."""
    found = []
    async for dialog in client.get_dialogs():
        chat = dialog.chat
        if chat and chat.type == enums.ChatType.PRIVATE and chat.is_deleted:
            found.append(chat.id)
    return found

async def remove_deleted_account_chats(client, chat_ids, limit):
    removed = 0
    ...
    try:
        ...
        async def handle_listeners(client, message):
            try:
                ...
                if message.chat and message.chat.type == enums.ChatType.PRIVATE and u.get("random_auto_reply_active", False) and message.from_user:
                ...
                if message.chat and message.chat.type == enums.ChatType.PRIVATE and u.get("auto_reply_active", False) and message.from_user:
        ...
        @ubot.on_message(filters.me & (filters.private | filters.group | filters.channel))
        async def handle_own_commands(client, message):
            try:
                u = await get_user(user_id)
                commands = u.get("own_commands", {})
                if not commands or not message.text:
                    return
                key = message.text.strip().lstrip(".").upper()
                ...
                if kind == "photo":
                    await client.send_photo(message.chat.id, content, caption=caption)
                elif kind == "video":
                    await client.send_video(message.chat.id, content, caption=caption)
                elif kind == "document":
                    await client.send_document(message.chat.id, content, caption=caption)
                else:
...
Choose an option below 👇"""
    ...
    btns = [
        ...
        [ibtn("Deleted Account Chats", "deleted_accounts_menu", style="danger", icon=BTN_EMOJIS["cross"])],
        [ibtn("Create Own Command", "own_commands_menu", style="primary", icon=BTN_EMOJIS["add"])],
...
{E_SETTING}"""
    ...
    elif data == "deleted_accounts_menu":
        u = await get_user(user_id)
        if not u.get("sessions"):
            return await safe_edit(query, f"{E_WRN} <b>No Connected Accounts!</b> Please add an account first.", {"inline_keyboard": [[ibtn("Add Account", "add_session", style="success", icon=BTN_EMOJIS["add"])], [ibtn("Back", "back_home", style="danger", icon=BTN_EMOJIS["cross"])]]})
        status = await api_send(user_id, f"{E_SYNC} <b>Scanning connected account chats...</b>")
        total = 0
        for session_str in u.get("sessions", []):
            try:
                scan_client = Client(f"deleted_scan_{user_id}_{int(time.time()*1000)}", api_id=API_ID, api_hash=API_HASH, session_string=session_str, in_memory=True)
                await scan_client.start()
                total += len(await find_deleted_account_chats(scan_client))
                await scan_client.stop()
            except Exception:
                pass
        await api_edit(user_id, status.id, f"{E_CHK} <b>Total deleted-account chats found:</b> <code>{total}</code>\n\nHow many deleted-account chats do you want to remove? Send a number, for example <code>100</code>.", {"inline_keyboard": [[ibtn("Cancel", "back_home", style="danger", icon=BTN_EMOJIS["cross"])]]})
        USER_STATES[user_id] = {"state": "WAITING_DELETED_ACCOUNT_COUNT", "available": total}
        return

    elif data == "own_commands_menu":
        u = await get_user(user_id)
        cmds = u.get("own_commands", {})
        listing = "\n".join(f"• <code>{k}</code> → {html.escape(str(v.get('content',''))[:40])}" for k,v in cmds.items()) or "No custom commands created yet."
        kb = {"inline_keyboard": [[ibtn("Create Command", "own_command_create", style="success", icon=BTN_EMOJIS["add"])], [ibtn("Back", "back_home", style="danger", icon=BTN_EMOJIS["cross"])]]}
        return await safe_edit(query, f"🛠 <b>Your Custom Commands</b>\n\n{listing}\n\nSend a command name and then its text/photo.", kb)

    elif data == "own_command_create":
        USER_STATES[user_id] = {"state": "WAITING_OWN_COMMAND_NAME"}
        return await safe_edit(query, "🛠 <b>Create Own Command</b>\n\nSend command name, example: <code>H6</code> or <code>HO</code>\n\nDo not include spaces.", {"inline_keyboard": [[ibtn("Cancel", "own_commands_menu", style="danger", icon=BTN_EMOJIS["cross"])]]})
...
Commands: /chk , /pause , /resume , /stop {E_SETTING}"""
        ...
        if state == "WAITING_OWN_COMMAND_NAME":
            name = (message.text or "").strip().lstrip(".").upper()
            if not re.fullmatch(r"[A-Z0-9_]{1,32}", name):
                return await api_send(user_id, "❌ Invalid command. Use only A-Z, 0-9 or underscore, e.g. <code>H6</code>.")
            USER_STATES[user_id] = {"state": "WAITING_OWN_COMMAND_CONTENT", "command_name": name}
            return await api_send(user_id, f"✅ Command <code>{name}</code> selected. Now send the text or photo/video/document with caption.")

        if state == "WAITING_OWN_COMMAND_CONTENT":
            name = USER_STATES[user_id].get("command_name")
            item = {"type": "text", "content": message.text or "", "caption": ""}
            if message.photo:
                item = {"type": "photo", "content": message.photo.file_id, "caption": message.caption or ""}
            elif message.video:
            ...
            if not item["content"]:
                return await api_send(user_id, "❌ Send text or supported media with caption.")
            u = await get_user(user_id)
            commands = u.get("own_commands", {})
            commands[name] = item
            await users_col.update_one({"_id": user_id}, {"$set": {"own_commands": commands}})
            USER_STATES.pop(user_id, None)
            return await api_send(user_id, f"✅ Custom command <code>{name}</code> saved. Use <code>{name}</code> in any chat.")

        if state == "WAITING_DELETED_ACCOUNT_COUNT":
            raw = (message.text or "").strip()
            if not raw.isdigit() or int(raw) <= 0:
                return await api_send(user_id, f"{E_WRN} Please send a valid positive number, e.g. <code>100</code>.")
            requested = int(raw)
            available = int(USER_STATES[user_id].get("available", 0))
            USER_STATES.pop(user_id, None)
            if available <= 0:
                return await api_send(user_id, f"{E_CHK} No deleted-account chats were found.")
            target = min(requested, available)
            u = await get_user(user_id)
            removed_total = 0
            for session_str in u.get("sessions", []):
                if removed_total >= target:
                    break
                try:
                    clean_client = Client(f"deleted_clean_{user_id}_{int(time.time()*1000)}", api_id=API_ID, api_hash=API_HASH, session_string=session_str, in_memory=True)
                    await clean_client.start()
                    deleted_ids = await find_deleted_account_chats(clean_client)
                    removed_total += await remove_deleted_account_chats(clean_client, deleted_ids, target - removed_total)
                    await clean_client.stop()
                except Exception:
                    pass
            return await api_send(user_id, f"{E_CHK} <b>Deleted-account cleanup completed.</b>\n\nRequested: <code>{requested}</code>\nFound: <code>{available}</code>\nRemoved: <code>{removed_total}</code>")
...
async def _start_render_health_server():
    """Keep Render Web Service healthy by listening on its assigned PORT."""
