...
async def start_bot():
    ...
    # Render Web Services require an open TCP port. This tiny local health
    # server keeps the service healthy while the Telegram bot polls normally.
    # It uses only Python's standard library, so no extra dependency is needed.
    import os
    port = int(os.environ.get("PORT", "10000"))

    async def _health_client(reader, writer):
        try:
    ...
    # Keep everything on this one event loop. Do not call pyrogram.idle().
    try:
        ...
        # Render can cancel the event loop while Pyrogram dispatcher workers
        # belong to a different loop. Do not call bot.stop() during that
        # cancellation; the process will be terminated safely by Render.

if __name__ == "__main__":
    # Keep the entire Pyrogram lifecycle on one asyncio event loop.
